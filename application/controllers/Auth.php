...
class Auth extends CI_Controller
{
	...
	public function users()
	{
		...
		} else if (!$this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
		{
			...
			$this->base->load('admin', 'user/users', $this->data);
	...
	public function register()
	{
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data)) {
			...
			$this->data['purok'] = [
				...
				'type' => 'text',
				'placeholder' => 'Purok',
				'value' => $this->form_validation->set_value('purok'),
			];
			$this->data['barangay'] = [
				'name' => 'barangay',
				'id' => 'barangay',
				...
				'value' => $this->form_validation->set_value('barangay'),
			];
			$this->data['city'] = [
				'name' => 'city',
				'id' => 'city',
				'class' => 'form-control',
				'type' => 'text',
				'value' => empty($this->form_validation->set_value('city')) ? 'Oroquieta City' : null,
			];
			$this->data['province'] = [
				'name' => 'province',
				'id' => 'province',
				'class' => 'form-control',
				'type' => 'text',
				'value' => empty($this->form_validation->set_value('province')) ? 'Misamis Occidental' : null,
			];
			$this->data['password'] = [
				'class' => 'form-control',
				'name' => 'password',
	...
	public function edit_user($id)
	{
		...
		$this->form_validation->set_rules('city', 'City or Municipality', 'trim|required');
		...
		$this->data['purok'] = [
			...
			'type' => 'text',
			'placeholder' => 'Purok',
			'value' => $this->form_validation->set_value('purok', $user->purok),
		];
		$this->data['barangay'] = [
			'name' => 'barangay',
			'id' => 'barangay',
			...
			'type' => 'text',
			'placeholder' => 'Barangay',
			'value' => $this->form_validation->set_value('barangay', $user->barangay),
		];
		$this->data['city'] = [
			'name' => 'city',
			'id' => 'city',
			'class' => 'form-control',
			'type' => 'text',
			'value' => empty($this->form_validation->set_value('city')) ? 'Oroquieta City' :  $user->city,
		];
		$this->data['province'] = [
			'name' => 'province',
			'id' => 'province',
			'class' => 'form-control',
			'type' => 'text',
			'value' => empty($this->form_validation->set_value('province')) ? 'Misamis Occidental' : $user->province,
		];
		$this->data['password'] = [
			'class' => 'form-control',
			'name' => 'password',
		...
		$this->base->load('client', 'auth/edit_user', $this->data);
	}
	/**
	 * Create a new user
	 */
	public function createUser()
	{
		...
		$this->form_validation->set_rules('identity', 'Username', 'required|min_length[5]|is_unique[users.username]');
		$this->form_validation->set_rules('first_name', 'First Name', 'trim|required');
		$this->form_validation->set_rules('last_name', 'Last Name', 'trim|required');
		$this->form_validation->set_rules('email', 'Email Address', 'trim|required|valid_email|is_unique[users.email]');
		$this->form_validation->set_rules('group', 'User Role', 'required');
		$this->form_validation->set_rules('password', 'Password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|matches[confirmpassword]');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($insert) {
				...
				$validator['msg'] = $this->ion_auth->messages();
			} else {
				$validator['success'] = false;
				$validator['msg'] = $this->ion_auth->messages();
			}
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			$this->auth_layout->load('auth', 'auth/login', $this->data);
		}
	}
	...
	public function getUser()
	{
		...
		$validator['data'] = $this->ion_auth->user($id)->row();
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->auth_layout->load('auth', 'auth/forgot_password', $this->data);
		} else {
			$identity_column = $this->config->item('identity', 'ion_auth');
	...
	public function user_profile($id)
	{
		...
		$this->base->load('admin', 'user/user_profile', $this->data);
	}
